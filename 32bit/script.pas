...
procedure SetVar(n, s: string);
  var
	...
	n := AllCaps(n);
  	c2 := 0;
	found := false;
	while (c2 <= vmax) and not(found) do begin
		if (v[c2]^.n <> n) then
			inc(c2)
		else
			found:=true;
	end;
{    	while (v[c2]^.n <> n) and (c2 <= vmax) do begin
		writeln(c2);
    		Inc(c2);
	end;}
		if c2 <= vmax then begin
			if s <> '' then
  	  			v[c2]^.p := s
		    	else
  		  		v[c2]^.n := '';
	...
	{		while (v[c2]^.n <> '') and (c2 <= vmax) do
      				Inc(c2);}
	      			if (c2 <= vmax) and (s <> '') then
  	    			begin
    	  	v[c2]^.n := n;
      	  v[c2]^.p := s;
	      end
...
procedure SetTrap(k: char; s: string);
  var
    ...
    begin
  	  ...
  	  begin
    	  c2 := 0;
      	while (t[c2]^.k <> #0) and (c2 <= tmax) do
      		Inc(c2);
	      if (c2 <= tmax) and (s <> '') then
  	    begin
    	  	t[c2]^.k := k;
                t[c2]^.l := s;
	      end
...
{  fn:=allcaps(fn);}
  ...
  if pos(DirectorySeparator,fn)=0 then fn:=adrv(systat^.nexecutepath)+fn;
