...
procedure cursoron(b:boolean);
begin
if (b) then nCursor(cON) else nCursor(cOFF);
end;
...
begin
  ...
  for x:=(x1+1) to (x2+1) do begin
    if (tpe=1) then 
      // mem[vidseg:(160*(c+1)+2*(x-1))+1]:=8
    else 
      ; //mem[vidseg:(160*(c+2)+2*(x-1))+1]:=8;
  end;
  textcolor(7);
end;
...
{Where vidseg is $B800 or $B000...notice the +1 on the end of the mem..I'm }
  ...
  if (linetype>0) then crt.window(TLX+1,TLY+1,BRX-1,BRY-1)
                  else crt.window(TLX,TLY,BRX,BRY);
...
begin
  ...
  crt.window(TLX,TLY,BRX,BRY);                 { set window size }
...
begin
  ...
  removewindow(wind);
  crt.window(oldx1,oldy1,oldx2,oldy2);
...
crt.window(TLX+1,TLY+1,BRX-1,BRY-1);
end;
