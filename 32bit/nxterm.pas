(*****************************************************************************)
(*>                                                                         <*)
(*>  NXTERM.PAS - Nexus Communications Program                              <*)
(*>                                                                         <*)
(*****************************************************************************)
{$A+,B+,D-,E+,F+,I+,L-,N-,O+,R-,S+,V-}
{$M 60000,0,50000}      { Memory Allocation Sizes }

program miniterm;

uses
  crt, dos, myio, file1, file0, common, tmpcom;
...
var c,bl,bl2:char;
  ...
  begin
    ...
    setwindow(wind,st,10,st+length(s)+5,14,9,1,1,'',FALSE);
...
(*    if ((c>=#224) and (c<=#226)) then docchk(c);*)
  ...
  begin
    savepos(sxx,syy);
    setwindow(wind,3,5,38,21,9,1,1,'',FALSE);
    ...
    if termprotocol<>-1 then begin
      i:='';
      setwindow(wind,3,10,77,16,9,1,1,'',FALSE);
      clearscr; tc(15);
      writeln;
      ...
      if pnumber=4 then begin
        ...
        if i<>'' then begin
          ...
          if ioresult<>0 then begin
            ...
            close(f);
            setwindow(wind,27,10,52,16,9,1,1,'',FaLSE);
            clearscr; tc(15);
            writeln;
...
*)
  ...
  var c,kk:char;
    ...
    var i,j:integer;
    begin
      if (rs='CONNECT') then spd:='300' else
      if (rs='CONNECT 1200') then spd:='1200' else
      if (rs='CONNECT 2400') then spd:='2400' else
      if (rs='CONNECT 4800') then spd:='4800' else
      if (rs='CONNECT 9600') then spd:='9600' else
      if (rs='CONNECT 14400') then spd:='14400' else
      if (rs='CONNECT 19200') then spd:='19200' else
      if (rs='CONNECT 38400') then spd:='38400';
      chkcom:=TRUE;
    end;
  ...
  begin
    cursoron(FALSE);
    savepos(sxx,syy);
    setwindow(wind,1,1,51,9,9,1,1,'',FALSE);
    clearscr; try:=0;
  ...
  begin
    ...
    cursoron(FALSE);
    savepos(sxx,syy);
    setwindow(wind,1,5,79,22,9,1,1,'',FALSE);
    showlist;
    done:=FALSE;
    repeat
      updatelist;
  ...
  procedure logo;
  begin
    clearscr; tc(1); box(1,11,1,68,5,'',3,0,TRUE); window(1,1,80,25);
    wcenter('NxTerm '+ver,15,2);
    wcenter('Copyright 1994 Intuitive Vision Software.  All Rights Reserved.,',11,3);
    wcenter('For help, press Alt-Z.',14,6);
    tc(7);
  end;
  ...
  begin
    ...
    setwindow(wind,43,1,80,18,4,0,1,'',FALSE);
    tc(15);
  ...
  begin
    ...
    enddayf:=FALSE; beepend:=FALSE; useron:=FALSE; chatcall:=FALSE;
...
{    iport;}
    ...
    savepos(x,y);
    setwindow(wind,1,1,50,8,9,1,1,'',FALSE);
    ...
    loading(start_dir+'\nxterm.fon');
    if not exist(start_dir+'\nxterm.fon') then begin
      assign(fil,start_dir+'\nxterm.fon');
      rewrite(fil);
      with ns[1] do begin
    ...
    loading(start_dir+'\nxterm.cfg');
    if not exist(start_dir+'\nxterm.cfg') then begin
      assign(cfgfil,start_dir+'\nxterm.cfg');
      rewrite(cfgfil);
      with mini do begin
        dpath:=start_dir+'\';
      end;
        ...
        if (keypressed) then begin
          ...
          case ord(c) of
            ...
            36:begin
                 i:=textattr;
                 savepos(sx,sy);
                 setwindow(wind,1,1,80,25,7,0,0,'',FALSE);
                 writeln('Type "EXIT" to return to NxTerm.');
...
begin
  ...
  close(systatf);
  if (not errs) then begin
    assign(modemrf,systat.gfilepath+'com0001.dat');
    {$I-} reset(modemrf); {$I+}
    errs:=(ioresult<>0);
  ...
  if (not errs) then begin
    ...
    if (not errs) then begin
      ...
      with thisuser do begin
        pagelen:=25;
        ac:=[ansi,color]; ac:=ac-[onekey,pause,novice];
      end;
    end;
    close(uf);
  end;
