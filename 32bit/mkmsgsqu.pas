...
Type SqMsgObj = Object(AbsMsgObj)
  ...
  Function  IsDirect: Boolean; Virtual; {Is current msg crash}
  ...
  Procedure SetDirect(St: Boolean); Virtual; {Set crash netmail status}
  ...
  Procedure YoursPrior; Virtual; {Seek next your mail}
...
Function SqMsgObj.IsHold: Boolean; {Is current msg local}
  Begin
...
Function SqMsgObj.IsDirect: Boolean; {Is current msg crash}
  begin
  IsDirect:=FALSE;
  end;
...
Procedure SqMsgObj.SetHold(St: Boolean); {Set local status}
  Begin
...
Procedure SqMsgObj.SetDirect(St: Boolean); {Set crash netmail status}
  Begin
  End;
...
Function SqMsgObj.GetMSGID:STRING;
  begin
...
Function SqMsgObj.GetHighActiveMsgNum: Longint;

begin
GetHighActiveMsgNum:=NumberOfMsgs;
end;
...
Procedure SqMsgObj.YoursFirst(MsgNum:Longint; Name: String; Handle: String);
  Begin
  SqInfo^.CurrIdx := MsgNum - 1;
  ReadIdx;
...
Procedure SqMsgObj.YoursPrior;
  ...
  WasFound := False;
  Dec(SqInfo^.CurrIdx);
  While ((SqInfo^.CurrIdx >=1) and (Not WasFound)) Do
    Begin
    While ((SqIdx^[SqInfo^.CurrIdx].Hash <> SqInfo^.HName) And
    (SqIdx^[SqInfo^.CurrIdx].Hash <> SqInfo^.HHandle) And
    (SqInfo^.CurrIdx <= SqInfo^.SqBase.NumMsg)) Do
