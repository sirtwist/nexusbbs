...
Common, common2, InitP,   mail6,  Menus2, File0,
  MyIO3,  Logon1,  Logon2,  NewUsers, WfcMenu, Menus,  keyunit, TmpCom,
...
{$ENDIF}
  ...
  // these two below are deprecated due to OS and should be removed.
...
{    end; }
    ...
    if (not doneday) then begin
      ...
      if (spd='KB') then begin
        ...
        pr1(#27+'[6n');
        st:=timer;
        while ((tcheck(st,5)) and (empty)) do begin 
	  timeslice; 
	end;
      ...
      if ((answerbaud<modemr^.minimumbaud) and (spd<>'KB')) then begin
        if ((modemr^.lockbegintime[getdow]<>0) or (modemr^.lockendtime[getdow]<>0)) then begin
          ...
          if (not hangup) then
            ...
            printf('MINIMUM');
            if (nofile) 
	      then print('Only callers connecting at |MINBAUD| baud and above may access this system.');
	  ...
	  sl1('!','GLOBAL.MNU Is Missing.  Skipped.');
	...
	menustack[i]:=langr.startmenu;
	    ...
	    sl1('!','PRELOGON.NPX Is Missing.  Skipped.');
    ...
    if (not doneday) and not(hangup) then begin
      ...
      with online do begin
        ...
        if (spd='KB') then begin
          ...
          if (modemr^.lockport) then 
	    lockbaud:=(modemr^.waitbaud div 10)
          else 
	    lockbaud:=0;
        end;
        if not(okansi) then 
    ...
    if (getuser) then begin
      ...
      //  schangewindow(TRUE,1);
        ...
        if (exist(adrv(systat^.nexecutepath)+'NEWUSER.NPX')) then begin
    ...
    if (not hangup) then begin
      ...
      if (not hangup) then begin
        with thisuser do begin
          ...
          if (testloop<0) or (testloop>32767) then 
	    testloop:=0;
    ...
    if not(doneday) then begin
      logoff;
      if tuser.name='' then 
        tuser.name:='No Caller';
      if tuser.realname='' then 
        tuser.realname:='No Caller';
      if (useron) then begin
      ...
      useron:=FALSE;
      if not(lcycle) and not(usewfcmenu) then 
        quitafterdone:=true;
