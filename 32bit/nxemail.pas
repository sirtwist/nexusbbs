...
uses dos,crt,myio,misc,mkmsgabs,mkopen,mkglobt,mkstring,mkdos,keyunit,
        fidonet;
...
istrigger:=result;
  ...
  begin
  case i of
        0:if (registered) then begin
            vtpword:=copy(build,1,3)+'-beta+ (public)';
          end else begin
            vtpword:=copy(build,1,3)+'-beta (public)';
          end;
        1:vtpword:=copy(build,1,length(build))+'-alpha';
        2:vtpword:=copy(build,1,length(build))+'-beta';
        3:vtpword:=copy(build,1,length(build))+'-dev';
        4:vtpword:=copy(build,1,length(build))+'-eep';
...
while (currentmsg^.seekfound) do begin
        ...
        newmsg^.setnextseealso(0);
          ...
          newmsg^.setpriv(TRUE);
          newmsg^.SetKillSent(TRUE);
          s3:=vtpword(ivr.rtype);
          if not(registered) then begin
                   s3:=s3+' (unlicensed)';
          end;
          if (registered) and (ivr.serial>0) then s3:=s3+' '+cstrf2(ivr.serial,value(copy(ivr.regdate,7,2)));
          ...
          newmsg^.doKludgeln(^A+'PID: nxEMAIL v'+ver2+s3);
          ...
          if (ioresult=0) then begin
                ...
                close(tf);
                newmsg^.dostringln('');
                s3:='--- nxEMAIL v'+ver2+vtpword(ivr.rtype);
