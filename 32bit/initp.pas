        ...
        'N':begin
            ...
            cnode:=value(cnd);
                        ...
                        if (cnode>1000) then begin
                                cwrite('%120%<%100%ù%120%> Node number not available with this license...%070%'+#13#10);
                                writeln;
...
{$ENDIF}
  ...
  if (ErrTemp <> 0) then begin
    cwrite('%120%<%100%ù%120%> Error #'+cstr(ErrTemp)+' opening '+adrv(systat^.gfilepath)+'SYSTEM.DAT... exiting.%070%'+#13#10);
    halt(exiterrors);
  end;
  ...
  ErrTemp := IOResult;
  if ErrTemp <> 0 then begin
    cwrite('%120%<%100%ù%120%> Error #'+cstr(ErrTemp)+' SYSTEM.DAT... exiting.%070%'+#13#10);
