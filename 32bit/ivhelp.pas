...
{$M 30000,0,30000}      { Memory Allocation Sizes }
program ivHelp;
...
function stripname(i:string):string;
var i1:string;
    n:integer;
...
function stripname2(i:string):string;
var i1:string;
    n:integer;
...
if (paramcount<2) then begin
        if (paramcount<1) then begin
                writeln('ivHELP [drive:][path]Helpfile [topic]');
                writeln;
                exit;
        end;
        ttemp:=3;
end else ttemp:=value(paramstr(2));
stemp:=paramstr(1);
helppath:=stripname2(stemp);
showhelp(stripname(stemp),ttemp,1,1,79,24,3,0,8,FALSE);
end.
