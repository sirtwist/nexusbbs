...
Function LongDate2(DStr: String): LongInt;
...
function extractword(str : string; n : integer) : string;
  ...
  While (i <= len) and (not done) do
    Begin
    While ((i <= len) and ((str[i] = #32) or (str[i] = ',') or (str[i] = #9) or (Str[i] = ';'))) do
      inc(i);
    if i <= len then
      inc(count);
    if count = n then
      ...
      If (i > 1) Then
        If Str[i-1] = ';' Then
          RetStr := ';';
      while ((i <= len) and ((str[i] <> #32) and (str[i] <> #9) and (Str[i] <> ';') and (str[i] <> ','))) do
        begin
        inc(retstr[0]);
...
{$ENDIF}
  ...
  HourStr := Copy(PadLeft(Long2Str(Dt.Hour),'0', 2),1,2);
  if (DT.Hour>12) then begin
        am:=FALSE;
        AHourStr:= Copy(PadLeft(Long2Str(Dt.Hour-12),'0', 2),1,2);
        APStr:='pm';
  end else begin
        AHourStr:= Copy(PadLeft(Long2Str(Dt.Hour),'0', 2),1,2);
        APStr:='am';
  end;
  ...
  CurrPos := Pos('AA', Mask);
  If CurrPos > 0 Then
    For i := 1 to Length(AHourStr) Do
      TmpStr[CurrPos + i - 1] := AHourStr[i];
  CurrPos := Pos('#', Mask);
  If CurrPos > 0 Then
    For i := 1 to Length(APStr) Do
      TmpStr[CurrPos + i - 1] := APStr[i];
  CurrPos := Pos('SS', Mask);
  If CurrPos > 0 Then
...
Function LongDate2(DStr: String): LongInt;
  ...
  DT.Month := Str2Long(Copy(DStr,1,2));
  DT.Day := Str2Long(Copy(DStr,4,2));
  DT.Hour := str2long(copy(dstr,10,2));
  DT.Min := str2long(copy(dstr,13,2));
  DT.Sec := str2long(copy(dstr,16,2));
  PackTime(DT, DosDate);
  LongDate2 := DosDate;
  End;
