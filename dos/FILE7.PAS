...
begin
  ...
  if (ioresult<>0) then begin
    if (useron) then sprint('%120%Disk error -  sorry, unable to upload it.');
    done:=TRUE; abort:=TRUE;
  end;
  prompti:=pynq('Do you want prompted input?');
  if (useron) then sprint('%070%Upload Ascii text.  Press Ctrl-Z (^Z) when done');
      ...
      if (not done) then begin
        ...
        repeat
          ...
          if (b=26) then begin
            ...
            nl;
            if (useron) then sprint('%150%End Of File Received');
          end else begin
        ...
        if (ioresult<>0) then begin
          nl;
          if (useron) then sprint('%120%Disk error');
          done:=TRUE; abort:=TRUE;
        end;
        inc(line_count);
  ...
  if (abort) then erase(f)
    ...
    sprint('%150%'+cstr(line_count)+' lines, '+cstr(kbyte_count)+'k uploaded');
...
begin
  ...
  if (ioresult<>0) then print('File not found.') else begin
    ...
    prompt(^Z);
    nl; nl;
    sprint('%150%File transmission complete.');
  end;
end;
