	...
	case anyerrors of
        ...
        1:begin
                ...
                case modemr^.ctype of
                       1:begin
                                sl1('!','Unable to detect active FOSSIL driver');
                                writeln('Unable to detect active FOSSIL driver -- exiting.');
                         end;
                       2:begin
                                sl1('!','Error Addressing Port '+cstr(modemr^.comport));
                                writeln('Error addressing com port '+cstr(modemr^.comport)+' -- exiting.');
                         end;
                       3:begin
                                sl1('!','Unable to detect DigiBoard');
                                writeln('Unable to detect DigiBoard -- exiting.');
                         end;
                end;
                halt(exiterrors);
	end;
...
begin
  ...
  case topscrnum of
        0:begin
          ...
          nodestr:=mln('Node '+cstr(cnode),10);
          with thisuser do begin
